...
async fn download_image(
    ...
    // filename: {author handle}/{created_at_localtime}_bsky_{post_id}_{image_no}.{ext}
    // example: bsky.app/20231223_035832_bsky_3kh5rarr3gn2n_01.jpeg
    ...
    let creation_time = DateTime::parse_from_rfc3339(&post.indexed_at)?;
    let creation_time_local: DateTime<Local> = DateTime::from(creation_time);
    let creation_time_str = creation_time_local.format("%Y%m%d_%H%M%S");
